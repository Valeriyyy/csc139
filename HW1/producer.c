...
CSC139 
Fall 2018
First Assignment PRODUCER
Kutsar, Valeriy
Section #2
OSs Tested on: athena
*/
...
int GetRand(int, int);
void validateBufSize(int);
void validateItemCnt(int, int);
void validateRandSeed(int);
...
void InitShm(int bufSize, int itemCnt)
{
    int in = 0;
    int out = 0;
    const char *name = "OS_HW1_KutsarValeriy"; // Name of shared memory object to be passed to shm_open
    ...
    gShmPtr = mmap(0, SHM_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, shmid, 0);
    if(shmid < 0) {
      perror("shmopen");
      exit(1);
    }
...
void Producer(int bufSize, int itemCnt, int randSeed)
{
    ...
    for(i = 0; i < itemCnt; i++){
      ...
      printf("producing item %d with vale %d at Index %d\n", i, val, in);
...
void validateBuffSize(int bufSize){
  if(bufSize < 0 || bufSize > 1000){
    printf("ERROR: BUFFER SIZE NOT IN RANGE(0-1000");
    exit(1);
  }
}
