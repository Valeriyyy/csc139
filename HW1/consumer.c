...
int main()
{
    const char *name = "OS_HW1_KutsarValeriy"; // Name of shared memory block to be passed to shm_open
	...
	// where i is the item number, val is the item value, out is its index in the bounded buffe
    int i;
    for(i = 0; i < itemCnt; i++){
        while(GetIn() == out);
